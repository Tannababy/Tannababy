# Hi there, I'm Lotanna 👋🏾

London based Full-Stack Developer with experience building web applications, APIs, and backend systems using JavaScript, Node.js, SQL, and Java.

I enjoy creating practical software solutions while continuously expanding my skills across modern web technologies, backend development, and automation.


### Currently Working With
- JavaScript
- Node.js & Express
- SQL
- Python

### Also Experienced In
- Java
- TypeScript
- Spring Boot
- REST APIs
- Object-Oriented Programming

### Currently Learning
- Next.js
- Advanced Node.js Backend Development
- Python Automation & Scripting
- Docker & CI/CD

## 🛠️ Tech Stack

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=git,html,css,sass,java,ts,js,scala,spring,nodejs,express,mysql,postman,docker" />
  </a>
</p>

## Projects

| Project | Tech Stack | Description | Link |
|----------|------------|-------------|------|
| Library Management System (console) | Java, JSON | Console-based application for managing books, users, and borrowing records. | [Repo](https://github.com/Tannababy/library-management-system) |
| Library API | Java, Spring Boot, MySQL | RESTful API with CRUD operations and persistent database storage. | [Repo](https://github.com/Tannababy/library-backend) |
| Tic Tac Toe | TypeScript | Browser-based game featuring modular architecture and score tracking. | [Repo](https://github.com/Tannababy/naughts-crosses-game) |
| Snap Game | Java | Two-player card game simulation implementing object-oriented design principles. | [Repo](https://github.com/Tannababy/cardGame-Snap) |

## 📊 GitHub Stats

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Tannababy&layout=compact)


## 📫 Connect With Me

- LinkedIn: [LinkedIn](www.linkedin.com/in/lotanna-amobi-b83750210)
- Email: tannaloamobi@gmail.com
